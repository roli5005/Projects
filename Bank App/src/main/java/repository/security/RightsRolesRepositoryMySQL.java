...
public class RightsRolesRepositoryMySQL implements RightsRolesRepository {
    ...
    public Role findRoleById(Long roleId) {
        ...
        try {
            ...
            roleResultSet.next();
            String roleTitle = roleResultSet.getString("role");
            return new Role(roleId, roleTitle, findRightsForRole(roleId));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Right> findRightsForRole(Long roleId) {
        ...
        try{
            PreparedStatement findStatement = connection.prepareStatement("SELECT * FROM `"+ROLE_RIGHT+ "` WHERE `role_id` = "+roleId);
            ResultSet resultSet = findStatement.executeQuery();
            while(resultSet.next())
            {
                Long right_id = resultSet.getLong("right_id");
                PreparedStatement getRightStatement = connection
                        .prepareStatement("SELECT * FROM `"+RIGHT +"` WHERE `id` = "+right_id);
                ResultSet resultSet1 = getRightStatement.executeQuery();

                if (resultSet1.next()){
                    Right right = new Right(right_id,resultSet1.getString("right"));
                    rights.add(right);
                }
